...
exports.sendMessage = async (req, res, next) => {
  ...
  if (!senderId)
    return res.status(400).json({
      message: `Please provide your Telnyx sender ID`,
    });

  if (!contacts.length)
    return res.status(400).json({
      message: `Please provide recipient's number`,
    });

  try {
    const sendSMS = async (message, senderId, contacts) => {
      const promises = contacts.map(async (contact) => {
        return await telnyx.messages.create({
          from: senderId, // Your Telnyx number
          to: contact,
          text: message,
        });
      });
    ...
    await sendSMS(message, senderId, contacts);
    ...
    return res.status(400).json({
      message: error.message || "Error sending your campaign",
      status: "fail",
    });
  }
};
